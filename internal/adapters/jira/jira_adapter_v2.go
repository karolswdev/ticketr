...
// NewJiraAdapterV2FromConfig creates a new V2 adapter from workspace configuration
func NewJiraAdapterV2FromConfig(config *domain.WorkspaceConfig, fieldMappings map[string]interface{}) (ports.JiraPort, error) {
	if config == nil {
		return nil, fmt.Errorf("[jira-v2] workspace configuration is required")
	}

	// Validate configuration
	if config.JiraURL == "" {
		return nil, fmt.Errorf("[jira-v2] Jira URL is required in workspace configuration")
	}
	if config.Username == "" {
		return nil, fmt.Errorf("[jira-v2] username is required in workspace configuration")
	}
	if config.APIToken == "" {
		return nil, fmt.Errorf("[jira-v2] API token is required in workspace configuration")
	}
	if config.ProjectKey == "" {
		return nil, fmt.Errorf("[jira-v2] project key is required in workspace configuration")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[jira-v2] failed to create Jira client: %w", err)
	}
...
func (j *JiraAdapterV2) Authenticate() error {
	...
	if err != nil {
		if resp != nil {
			return fmt.Errorf("[jira-v2] authentication failed with status %d: %w", resp.StatusCode, err)
		}
		return fmt.Errorf("[jira-v2] authentication failed: %w", err)
	}
	return nil
}
...
func (j *JiraAdapterV2) SearchTickets(ctx context.Context, projectKey string, jql string, progressCallback ports.JiraProgressCallback) ([]domain.Ticket, error) {
	...
	for {
		...
		if err != nil {
			if resp != nil {
				return nil, fmt.Errorf("[jira-v2] search failed with status %d: %w", resp.StatusCode, err)
			}
			return nil, fmt.Errorf("[jira-v2] search failed: %w", err)
		}
...
func (j *JiraAdapterV2) fetchSubtasks(ctx context.Context, parentKey string) ([]domain.Task, error) {
	// Check for cancellation
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	...
	if err != nil {
		if resp != nil {
			return nil, fmt.Errorf("[jira-v2] subtask search failed with status %d: %w", resp.StatusCode, err)
		}
		return nil, fmt.Errorf("[jira-v2] subtask search failed: %w", err)
	}
...
func (j *JiraAdapterV2) CreateTicket(ticket domain.Ticket) (string, error) {
	...
	if err != nil {
		if resp != nil {
			return "", fmt.Errorf("[jira-v2] create failed with status %d: %w", resp.StatusCode, err)
		}
		return "", fmt.Errorf("[jira-v2] create failed: %w", err)
	}
...
func (j *JiraAdapterV2) UpdateTicket(ticket domain.Ticket) error {
	if ticket.JiraID == "" {
		return fmt.Errorf("[jira-v2] ticket does not have a Jira ID")
	}
	...
	if err != nil {
		if resp != nil {
			return fmt.Errorf("[jira-v2] get issue failed with status %d: %w", resp.StatusCode, err)
		}
		return fmt.Errorf("[jira-v2] get issue failed: %w", err)
	}
	...
	if err != nil {
		if resp != nil {
			return fmt.Errorf("[jira-v2] update failed with status %d: %w", resp.StatusCode, err)
		}
		return fmt.Errorf("[jira-v2] update failed: %w", err)
	}
...
func (j *JiraAdapterV2) CreateTask(task domain.Task, parentID string) (string, error) {
	...
	if err != nil {
		if resp != nil {
			return "", fmt.Errorf("[jira-v2] create subtask failed with status %d: %w", resp.StatusCode, err)
		}
		return "", fmt.Errorf("[jira-v2] create subtask failed: %w", err)
	}
...
func (j *JiraAdapterV2) UpdateTask(task domain.Task) error {
	if task.JiraID == "" {
		return fmt.Errorf("[jira-v2] task does not have a Jira ID")
	}
	...
	if err != nil {
		if resp != nil {
			return fmt.Errorf("[jira-v2] update subtask failed with status %d: %w", resp.StatusCode, err)
		}
		return fmt.Errorf("[jira-v2] update subtask failed: %w", err)
	}
...
func (j *JiraAdapterV2) GetProjectIssueTypes() (map[string][]string, error) {
	...
	if err != nil {
		if resp != nil {
			return nil, fmt.Errorf("[jira-v2] get project failed with status %d: %w", resp.StatusCode, err)
		}
		return nil, fmt.Errorf("[jira-v2] get project failed: %w", err)
	}
...
func (j *JiraAdapterV2) GetIssueTypeFields(issueTypeName string) (map[string]interface{}, error) {
	...
	if err != nil {
		if resp != nil {
			return nil, fmt.Errorf("[jira-v2] get create metadata failed with status %d: %w", resp.StatusCode, err)
		}
		return nil, fmt.Errorf("[jira-v2] get create metadata failed: %w", err)
	}

	// Find our project using the helper method
	targetProject := createMeta.GetProjectWithKey(j.projectKey)
	if targetProject == nil {
		return nil, fmt.Errorf("[jira-v2] project %s not found in metadata", j.projectKey)
	}
	...
	if targetIssueType == nil {
		...
		return nil, fmt.Errorf("[jira-v2] issue type '%s' not found. Available types: %v", issueTypeName, availableTypes)
	}
...
func (j *JiraAdapterV2) getJiraFieldID(humanName string) string {
...
func (j *JiraAdapterV2) applyCustomFields(fields *jira.IssueFields, customFields map[string]string) {
...
func (j *JiraAdapterV2) createReverseFieldMapping() map[string]string {
...
func (j *JiraAdapterV2) formatFieldValue(value interface{}) string {
	switch v := value.(type) {
	case string:
		return v
	case float64:
